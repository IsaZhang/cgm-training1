...
**不要使用本 skill 执行部署。** 项目的生产环境已从老服务器（`8.131.113.38`，pm2 管理）迁移到新服务器（`47.95.6.33`，Docker 容器 `cgm-training`）。如果按下方流程跑 `./deploy-quick-with-web.sh`，代码会部署到**已不再承载生产流量的机器**，新服务器不会更新。
